...
public class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerViewAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        ...
        holder.product_satisfy.setText("별"+ mProducts.get(position).getItemGrade()+"/5");
        holder.product_cnt_review.setText(" | 리뷰"+mProducts.get(position).getReviewCnt());
        holder.parentLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                LogWrapper.v(mContext.getClass().getSimpleName(), "btn_click", position + "st_product");
        ...
        holder.container.setAccessibilityDelegate(new View.AccessibilityDelegate() {
            ...
            public void onPopulateAccessibilityEvent(View host, AccessibilityEvent event) {
                ...
                if(event.getEventType() == AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUSED) {
                    LogWrapper.v(mContext.getClass().getSimpleName(), "current_focus", position + "st_product");
